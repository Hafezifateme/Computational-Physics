...
int h = 25;                    // external field
float J = +1;                     // spin-spin coupling constant
float T = 2.25;                    // Tempreture
...
float Delta_E_Flip(int i, int j, float H) {
    float sum = 0.0;
...
void Single_Monte_Carlo_Step(float H) {
...
void Execute(float H) {


for (int b=0; b<step ; b++)
{

Single_Monte_Carlo_Step(H);
...
<< "set autoscale y" "\n"  ;
 ...
 temp << "plot '-' using 2:1 title \"My Plot\" with lp" << endl;
    ...
    for( int b=-h; b<h; b++)
        temp << deltaM[b+h]*(1.0/(L*L)) << '\t' << b*(1.0/5) << endl;
